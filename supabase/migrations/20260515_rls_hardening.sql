-- ==========================================
-- BKY SISTEMI GUVENLIK SERTLESTIRME PAKETI
-- ==========================================

-- 1. TRANSACTIONAL RPC FONKSIYONLARI
-- ------------------------------------------

-- TİP GÜVENLİ ATAMA TRANSACTION (RPC)
...
RETURNS VOID AS $$
BEGIN
  DELETE FROM kullanici_olcut_atamalari
  WHERE user_id = p_user_id AND donem_id = p_donem_id;
...
-- BİRİM ATAMALARI İÇİN KISMİ (PARTIAL) SYNC RPC
...
BEGIN
  ...
  WHERE user_id = p_user_id 
    AND donem_id = p_donem_id 
    AND alt_olcut_id = ANY(p_scope_olcut_ids);

  DELETE FROM kullanici_olcut_atamalari
  WHERE donem_id = p_donem_id 
    AND alt_olcut_id = ANY(p_selected_olcut_ids)
...
RETURNS VOID AS $$
BEGIN
  DELETE FROM baslik_koordinatorleri
  WHERE kullanici_id = p_user_id;

  INSERT INTO baslik_koordinatorleri (kullanici_id, baslik)
  VALUES (p_user_id, p_baslik);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
CREATE POLICY "Users can view own assignments" ON kullanici_olcut_atamalari
FOR SELECT USING (auth.uid() = user_id OR EXISTS (SELECT 1 FROM profiller WHERE id = auth.uid() AND (rol ILIKE '%admin%' OR rol ILIKE '%yönetici%' OR rol ILIKE '%yonetici%')));

DROP POLICY IF EXISTS "Admins can modify assignments" ON kullanici_olcut_atamalari;
CREATE POLICY "Admins can modify assignments" ON kullanici_olcut_atamalari
FOR ALL USING (EXISTS (SELECT 1 FROM profiller WHERE id = auth.uid() AND (rol ILIKE '%admin%' OR rol ILIKE '%yönetici%' OR rol ILIKE '%yonetici%')));
